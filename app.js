...
function initApp() {
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            initPrayerTimes(); 
            initQuranList(); // Populate Reciters
        } else {
            currentUser = null;
...
async function fetchPrayerTimes(lat, lng) {
    ...
    try {
        ...
        if(data.code === 200) { prayerTimes = data.data.timings; updatePrayerUI(); }
...
function updateDateUI() {
    ...
    if(ro) {
        document.getElementById('tasks-container').classList.add('opacity-75', 'pointer-events-none');
        document.querySelector('.read-only-badge').style.display = 'inline-flex';
    } else {
        document.getElementById('tasks-container').classList.remove('opacity-75', 'pointer-events-none');
...
function renderMainUI(data) {
    ...
    for(const [k, v] of Object.entries(data.prayers)) {
        html += `<div onclick="toggleTask('prayers','${k}',${!v})" class="bg-white p-3 rounded-xl border flex items-center justify-between cursor-pointer ${v?'border-green-500 bg-green-50':'border-gray-100'}"><div><p class="font-bold text-xs ${v?'text-green-700':'text-gray-600'}">${pMap[k]}</p><p id="time-${k}" class="text-[10px] text-gray-400">--:--</p></div>${v ? '<i data-lucide="check-circle" class="w-4 h-4 text-green-600"></i>' : '<i data-lucide="circle" class="w-4 h-4 text-gray-300"></i>'}</div>`;
    }
    const q = data.quran || false;
    html += `<div onclick="toggleTask('root','quran',${!q})" class="bg-white p-3 rounded-xl border flex items-center justify-between cursor-pointer ${q?'border-green-500 bg-green-50':'border-gray-100'}"><div><p class="font-bold text-xs ${q?'text-green-700':'text-gray-600'}">ورد القرآن</p><span class="text-[9px] text-blue-500" onclick="event.stopPropagation(); toggleQuranModal()">📖 قراءة</span></div>${q ? '<i data-lucide="check-circle" class="w-4 h-4 text-green-600"></i>' : '<i data-lucide="circle" class="w-4 h-4 text-gray-300"></i>'}</div>`;
    html += `</div>`;
    ...
    for(const [k, meta] of Object.entries(HABITS_META)) {
        if(settings[k]) {
            ...
            html += `<div onclick="toggleTask('habits','${k}',${!v})" class="bg-white p-3 rounded-xl border flex items-center gap-2 cursor-pointer ${v?'border-yellow-400 bg-yellow-50':'border-gray-100'}"><i data-lucide="${meta.icon}" class="w-3 h-3 ${v?'text-yellow-600':'text-gray-400'}"></i><span class="text-[10px] font-bold ${v?'text-yellow-700':'text-gray-600'}">${meta.name}</span></div>`;
        }
    }
    html += `</div>`;
    pc.innerHTML = html;
    ...
    (data.customAdhkar || []).forEach((a, i) => {
        ...
        ac.innerHTML += `<div class="bg-white p-2 rounded-xl border border-gray-100 relative overflow-hidden"><div class="flex justify-between items-center mb-1"><span class="text-[10px] font-bold truncate">${a.name}</span><button onclick="removeAdhkar(${i})" class="text-red-300"><i data-lucide="x" class="w-3 h-3"></i></button></div><div class="flex justify-between items-end relative z-10"><div class="flex items-center gap-1"><span class="text-lg font-bold text-blue-600">${a.count}</span><button onclick="openManualCountModal(${i})" class="text-gray-300"><i data-lucide="edit-2" class="w-3 h-3"></i></button></div><button onclick="incrementAdhkar(${i})" class="w-7 h-7 bg-blue-600 text-white rounded-lg flex items-center justify-center click-anim"><i data-lucide="plus" class="w-3 h-3"></i></button></div><div class="absolute bottom-0 left-0 h-1 bg-blue-100 w-full"><div style="width:${pct}%" class="h-full bg-blue-500"></div></div></div>`;
    });
...
async function generateReport(period) {
    document.querySelectorAll('.report-tab').forEach(t => {
        ...
        else { t.classList.replace('text-[#047857]', 'text-gray-600'); t.classList.remove('bg-white', 'shadow-sm'); }
    });
    
    if (period === 'day') {
        document.getElementById('report-title-text').innerText = "تقرير يومي";
        if(lastUserData) {
            const p = document.getElementById('chart-percent').innerText;
            const a = document.getElementById('total-adhkar-count').innerText;
            document.getElementById('report-percent').innerText = p;
            document.getElementById('report-adhkar').innerText = a;
            renderReportChart('day', { percent: parseInt(p) });
        }
    } else {
        const days = period === 'week' ? 7 : 30;
        document.getElementById('report-title-text').innerText = period === 'week' ? "تقرير أسبوعي" : "تقرير شهري";
...
function renderReportChart(period, data) {
    ...
    if (period === 'day') {
        ...
        reportChartInstance = new Chart(ctx, { type: 'doughnut', data: { datasets: [{ data: [p, 100-p], backgroundColor: ['#047857', '#eee'], borderWidth: 0 }] }, options: { cutout: '70%', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
    } else {
        reportChartInstance = new Chart(ctx, { type: 'bar', data: { labels: data.history.map(h => h.label), datasets: [{ label: 'إنجاز', data: data.history.map(h => h.val), backgroundColor: '#047857', borderRadius: 2 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100, display: false }, x: { display: false } }, plugins: { legend: { display: false } } } });
    }
}
...
function downloadAsPDF() { const element = document.getElementById('report-preview-content'); const { jsPDF } = window.jspdf; html2canvas(element, { scale: 2 }).then(canvas => { const imgData = canvas.toDataURL('image/png'); const pdf = new jsPDF('p', 'mm', 'a4'); const pdfWidth = pdf.internal.pageSize.getWidth(); const pdfHeight = (canvas.height * pdfWidth) / canvas.width; pdf.addImage(imgData, 'PNG', 0, 10, pdfWidth, pdfHeight); pdf.save('report.pdf'); }); }
...
function downloadAsExcel() { /* kept simple */ const ws = XLSX.utils.json_to_sheet([{Date: getReadableDate(currentDate)}]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Report"); XLSX.writeFile(wb, 'report.xlsx'); }